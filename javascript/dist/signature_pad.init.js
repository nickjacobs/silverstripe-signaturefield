 ...
 */ class $d7070a1a72da4ff8$var$Point {
    distanceTo(start) {
...
class $d7070a1a72da4ff8$var$Bezier {
    static fromPoints(points, widths) {
        const c2 = this.calculateControlPoints(points[0], points[1], points[2]).c2;
        const c3 = this.calculateControlPoints(points[1], points[2], points[3]).c1;
        return new $d7070a1a72da4ff8$var$Bezier(points[1], c2, c3, points[2], widths.start, widths.end);
    }
    static calculateControlPoints(s1, s2, s3) {
        ...
        const ty = s2.y - cm.y;
        return {
            c1: new $d7070a1a72da4ff8$var$Point(m1.x + tx, m1.y + ty),
            c2: new $d7070a1a72da4ff8$var$Point(m2.x + tx, m2.y + ty)
        };
    }
    length() {
        const steps = 10;
...
class $d7070a1a72da4ff8$var$SignatureEventTarget {
...
function $d7070a1a72da4ff8$var$throttle(fn, wait = 250) {
    let previous = 0;
    let timeout = null;
    let result;
    let storedContext;
...
class $d7070a1a72da4ff8$export$2e2bcd8739ae039 extends $d7070a1a72da4ff8$var$SignatureEventTarget {
    clear() {
        const { _ctx: ctx , canvas: canvas  } = this;
    ...
    fromData(pointGroups, { clear: clear = true  } = {}) {
        if (clear) this.clear();
    ...
    _createPoint(x, y, pressure) {
        ...
        return new $d7070a1a72da4ff8$var$Point(x - rect.left, y - rect.top, pressure, new Date().getTime());
    }
    _addPoint(point, options) {
        const { _lastPoints: _lastPoints  } = this;
        _lastPoints.push(point);
        if (_lastPoints.length > 2) {
            if (_lastPoints.length === 3) _lastPoints.unshift(_lastPoints[0]);
            const widths = this._calculateCurveWidths(_lastPoints[1], _lastPoints[2], options);
            const curve = $d7070a1a72da4ff8$var$Bezier.fromPoints(_lastPoints, widths);
            _lastPoints.shift();
            return curve;
        }
        return null;
    ...
    _fromData(pointGroups, drawCurve, drawDot) {
        for (const group of pointGroups){
            const { points: points  } = group;
            const pointGroupOptions = this._getPointGroupOptions(group);
            if (points.length > 1) for(let j = 0; j < points.length; j += 1){
                const basicPoint = points[j];
                const point = new $d7070a1a72da4ff8$var$Point(basicPoint.x, basicPoint.y, basicPoint.pressure, basicPoint.time);
    ...
    toSVG({ includeBackgroundColor: includeBackgroundColor = false  } = {}) {
        ...
        this._fromData(pointGroups, (curve, { penColor: penColor  })=>{
        ...
        }, (point, { penColor: penColor , dotSize: dotSize , minWidth: minWidth , maxWidth: maxWidth  })=>{
    ...
    constructor(canvas, options = {}){
        ...
        this._handleMouseMove = (event)=>{
            if (this._drawningStroke) this._strokeMoveUpdate(event);
        };
        ...
        this._strokeMoveUpdate = this.throttle ? $d7070a1a72da4ff8$var$throttle($d7070a1a72da4ff8$export$2e2bcd8739ae039.prototype._strokeUpdate, this.throttle) : $d7070a1a72da4ff8$export$2e2bcd8739ae039.prototype._strokeUpdate;
